...
sys.path.insert(0, './segment_particles')
...
input_dir_root  = "./segment_particles/data/CICS_experiment/raw_image_data/"
output_dir_root = "./segment_particles/data/CICS_experiment/image_data/end_to_end_test/"
classification_folder_name = "classification/"

detection_radius = 20 # Radius (measured in pixels) that indicates the allowable distance between a predicted particle and a reference particle to be deemed accurate. Used on the orignal image
output_crop_size = 52 # The output size of the crops, measured in pixels. Used on the original image. 
...
def crop_particles_into_class_folders(original_image, predicted_image, particle_list):
	...
	for index_pred, centroid_pred in enumerate(pred_centroids_list): 
		...
		file_name = target_truth_metadata["main"] + "_" + target_truth_metadata["sub"] + "_" + str(int(centroid_pred_upscaled[0])) + "_" + str(int(centroid_pred_upscaled[0])) + ".bmp"
